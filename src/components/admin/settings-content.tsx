    ...
    <div className="space-y-8 max-w-md">
      {mode === "admin" && (
        <form action={dispatch} className="space-y-4">
          <div>
            <Label htmlFor="defaultAllowance">
            ...
            {state.errors?.defaultAllowance && (
              <p className="text-sm font-medium text-destructive mt-1">
                {state.errors.defaultAllowance[0]}
              </p>
            )}
          </div>
      ...
      {mode === "dev" && (
        <div className="space-y-4 pt-4 border-t">
          ...
          <p className="text-sm text-muted-foreground">
            Select a user to act as the currently logged-in user for prototyping
            purposes.
          </p>
          <div>
              ...
              onValueChange={(userId) => {
                if (userId) {
                  toast({
                    ...
                    description: `Changing active user. The page will reload.`,
                  });
                  switchUser(userId);
                }
              }}
